...
const char* MuonFile="../Output/ReconData/ReconMuonsData_WCT_t4_Tx-.root";
const char* FigureFile="../pic/AngularDistribution_WCT_t4_ChiSquare250.pdf";
const char* ModelFile="../Output/ReconMC/D645_WCT_t4.5/ReconMC_k20.root";
const int PMTLimit[2]={56,56}; // only use muons with lighted PMT number in this range
...
int ReadMuonList(vector<MuonIndex_t> &MuonIndex) {
    ...
    while (getline(infile, line)) {
        ...
        if (iss >> muonIndex.RunNo >> muonIndex.FileNo >> muonIndex.TriggerNo 
...
int main() {
    ...
    // if (nMuons != tMuon->GetEntries()) {
    //     cerr << "Error: Muon count mismatch between muon list and data file." << endl;
    //     return 1;
    // }

    double costheta,phi,MinChi2;
    tMuon->SetBranchAddress("costheta_rec",&costheta);
    tMuon->SetBranchAddress("phi_rec",&phi);
    tMuon->SetBranchAddress("MinChi2",&MinChi2);
    ...
    for (int ientry=0; ientry<tMuon->GetEntries(); ientry++){
        ...
        // MuonIndex_t muon = MuonIndex[ientry];
        // if (muon.RunNo != muon.RunNo || muon.FileNo != muon.FileNo || muon.TriggerNo != muon.TriggerNo) {
        //     cerr << "Error: Muon identity mismatch at entry " << ientry << endl;
        //     return 1;
        // }
        // if (muon.nLightedPMT < PMTLimit[0] || muon.nLightedPMT > PMTLimit[1]) continue; // apply cut on number of lighted PMTs
        // if (muon.TotalPE < 700) continue; // apply cut on total PE
        if (MinChi2 > 250) continue; 
        phi -= TMath::Pi() / 180*phi_dispalce;
        if (phi>TMath::Pi()) phi -= 2*TMath::Pi();
        if (phi<-TMath::Pi()) phi+= 2*TMath::Pi();
        // if (costheta>0.95){
        //     h_data->Fill(costheta,phi*180/TMath::Pi(),1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()+60,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()-60,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()+120,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()-120,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()+180,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()-180,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()+240,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()-240,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()+300,1./6);
        //     h_data->Fill(costheta,phi*180/TMath::Pi()-300,1./6);
        //     continue;
        // }
        h_data->Fill(costheta,phi*180/TMath::Pi());
        // cout << "Filling entry " << ientry << ": nLightedPMT=" << muon.nLightedPMT <<" costheta="<<costheta<<" phi="<<phi*180/TMath::Pi()<< endl;
    }
    ...
    h_data->Draw("col,text");
    h_data->SaveAs("../temp_PE700.root");
    // Add Pvalue
