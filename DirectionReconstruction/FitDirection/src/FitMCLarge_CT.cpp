...
void ReadMap(const char* templatefile, vector<node> &MeanMap)
{
    ...
    for (int i = 0; i < readTemplates; i++)
    {
        ...
        for (int j = 0; j < NChannels; j++)
        {
            ...
            if (!IsBadChannel(j, &d1.ChannelId)) continue; // only use the channels in ChannelId
            sumtime += Time2Mean[j];
            sumcharge += Energy[j];
...
int main(int argc, char **argv)
{
    ...
    for (int j = 0; j < NChannels; j++)
    {
        ...
        cout << "Time Shift for channel "<<j<<": "<<TimeShift[j]<<endl;
    }
    double error = eta; // Gain Calibration uncertainty;
    for (int j = 0; j < NChannels; j++)
    {
    ...
    for (int i = 0; i < aentry; i++)
    {
     ...
     //   if(nChannels<NChannels)
        ...
        double meanTime2  = 0;
        double meanPE2 = 0;
        std::vector<pair<double,int>> tVector;  //the data if channel risetime and channel ID
        for (int j = 0; j < NChannels; j++)
        {   
            if (IsBadChannel(j, BadChannelList)) continue;
            ...
            Energy[j] = (Energy[j] - chargeMean) / sqrt(chargeVariance); // shift to mean charge and normalize by variance
        }
        ...
        for (auto &&item : MeanMap)
        {
            ...
            {   
                ...
                chi2_charge += pow(Energy[ch] - (item.Energy)[ch], 2);
            }
            chi2 = chi2_time + chi2_charge;
            if (chi2>1e7){
                  cout << "Event " << i << ": nLightedPMT=" << nLightedPMT << ", ChannelId: "<<", chi2 "<<chi2<<endl;
            }
