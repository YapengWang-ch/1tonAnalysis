          ...
          // ID=(*ChannelId)[j];
            ...
            if (!peaks.empty()){
                ...
                for (int ll=0; ll<peaks.size(); ll++){
                  printf(" %d/%.2f",peaks[ll],baseline-(*waveform)[peaks[ll] + j*FormLength]);
                }  
                printf("\n");
            }else{
      ...
      return 1;
        ...
        for (int j=0; j<ChannelN; j++){
            ...
            if (!peaks.empty()){
                ...
                for (int ll=0; ll<peaks.size(); ll++){
                  printf(" %d/%.2f",peaks[ll],baseline-(*waveform)[peaks[ll] + j*FormLength]);
                  peaklist.push_back((*waveform)[peaks[ll] + j*FormLength]-baseline);
                }  
                printf("\n");
            }else{
                printf("peaks not found\n");
...
void PrintUsage(){
    std::cout<<"Usage: ./plotMC <input file> <output file> [<entry>] [--triggerNo triggerNo] [--title title] [--channel ch] \n";
