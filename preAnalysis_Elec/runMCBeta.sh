...
logfile="$HOME/1ton/ReConstruction/MuonFlux/preAnalysis1.log"
rm $logfile
# subdir="InsidePMT"
# mkdir -p /mnt/stage/wangyp/Simulation/new1ton/water/templates/PreAnalysis_plate
inputpath="/mnt/stage/wangyp/Simulation/new1ton/water/CaliTest/BetaAll10MeV"
outputpath="/mnt/stage/wangyp/Simulation/new1ton/water/CaliTest/PreAnalysis_BetaAll_10MeV"
mkdir -p $outputpath
...
generate_commands() {
    for i in $(seq 0 0); do
        for j in $(seq 0 100); do
            if [ "$j" -eq 0 ]; then
                filename="${inputpath}/Beta_10MeV_All.root"
            else
                filename="${inputpath}/Beta_10MeV_All_${j}.root"
            fi

            prepath="${outputpath}/Beta_10MeV_All_${j}.root"
