...
peaksfile="/mnt/stage/wangyp/PreAnalysis/new1ton/water/LSpeaks2"
# peaksfile="../output/peaks3"
outputpath="../output/LSCali2w"
histpath="../output/LSpeaks2"
logfile="../log"
...
rundata() {
    ...
    if [ -d "$runfile" ]; then
        ...
        if [ -e "${peaks}" ]; then
            ./TimeCali2w "${peaks}" "${output}" >> "${runlog}"
