...
const double sigma_TCali=1; // the error of Time Calibration
...
int main(int argc, char **argv)
{
    ...
    {
        cout << "Usage:" << endl;
        cout << "  ./FitMCLarge inputFilename outputFilename [Int_t knn] [float Tc]" << endl;
        cout << endl;
        return 1;
    }
    ...
    for (int i = 0; i < aentry; i++)
    {
     ...
     //   if(nChannels<NChannels)
        ...
        for (auto &&item : MeanMap)
        {
            ...
            {   
                if (IsBadChannel(j,BadChannelList)) continue;
                // chi2_time += pow(Time1[j] - (item.Time2Mean)[j], 2) * std::sqrt(Energy[j]*(item.Energy)[j]) / ( item.TimeRange * timeRange);
                // chi2_charge += pow(Energy[j] - (item.Energy)[j], 2) ;
                chi2_time += pow(Time1[j] - (item.Time2Mean)[j], 2) * std::sqrt(Energy[j]*(item.Energy)[j]/(1+Energy[j]*meanPE1*sigma*sigma/Ts2)) ;
                chi2_charge += Tc*Tc*pow(Energy[j] - (item.Energy)[j], 2)/std::sqrt(Energy[j]*(item.Energy)[j]) ;
            }
