...
def compare_files(file1, file2,file3):
    ...
    plt.errorbar(common_ch, norm_mean1, fmt='s', markerfacecolor='none', color='b', 
                 ecolor='lightgray', elinewidth=2, capsize=4, label='LS C',)
    plt.errorbar(common_ch, norm_mean2, yerr=norm_err2, fmt='o', color='r', 
                 ecolor='lightgray', elinewidth=2, capsize=4, label='LS W')
    plt.errorbar(common_ch, norm_mean3, yerr=norm_err3, fmt='s', color='g', 
                 ecolor='lightgray', elinewidth=2, capsize=4, label='Water W')
    plt.plot(common_ch, norm_mean1, color='b', linestyle='-', alpha=0.6)
    plt.plot(common_ch, norm_mean2, color='r', linestyle='-', alpha=0.6)
    plt.plot(common_ch, norm_mean3, color='g', linestyle='-', alpha=0.6)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python compare_channels.py <file1.txt> <file2.txt> <file3>")
        sys.exit(1)
    
    file1 = sys.argv[1]
    file2 = sys.argv[2]
    compare_files(file1, file2, sys.argv[3])