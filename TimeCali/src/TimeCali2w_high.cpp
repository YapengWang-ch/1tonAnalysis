...
// vector<int> badchannellist = {5,11,18,26,29,40,51,53,58,2,10,18,26,34,42,50,58};
...
void analyze_darknoise(const char* filename, const char* outputpath) {
    ...
    for (int refchannel=0; refchannel < 60; refchannel ++){
        ...
        for (Long64_t i = 0; i < nEntries; i++) {
            ...
            if (peaks == nullptr || peaks->empty()) continue; // 跳过空的事例
            ...
            for (size_t j = 0; j < peaks->size(); j++) {
                ...
                if ((channel==refchannel)&& (peaks->at(j).charge/gain[channel]>3)){
            ...
            for (size_t j = 0; j < peaks->size(); j++) {
                ...
                if (peaks->at(j).charge<1000) continue; 
        ...
        for (int ch = refchannel + 1; ch < 60; ++ch) {
            ...
            double lowBound = Mean - 5.0;
            double highBound = Mean + 5.0;
