...
int GetDN(const char* inputdir,  const char* outputpath){
    ...
    TH1F *h1=new TH1F("peakPosition","peakPosition",200,0,200);
    TH2F *h2=new TH2F("peakChargeVsHeight","peakChargeVsHeight",200,0,500,200,0,1e4);
    // class PeakInfo peak;
    // int FormLength;
    double lastsecond=0;
    // double timeinterval;
    ...
    for(long i=0; i<EN; i++){
        ...
        for (auto &peak:*peaks){
            if (peak.peakLoc>120 || peak.peakLoc<10) continue;
            h1->Fill(peak.peakLoc);
            h2->Fill(peak.peakHeight,peak.charge);
            if (GetCharge(charge,peak)){
                // if (peak.secondpeaktime>0){
                //     hT->Fill(peak.secondpeaktime-peak.peakLoc);
                // }else{
                //     hT->Fill(499);
                // }
                // hT->Fill(peak.peakLoc);
                darknoise.emplace_back(peak.channel,charge);
                // cout << "peak in linear area found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" charge:"<<peak.charge<<" height:"<<peak.peakHeight<<" ratio:"<<peak.charge/peak.peakHeight<<" risetime:"<<peak.risetime<<" loc:"<<peak.peakLoc<<endl;
            }
            // if (peak.risetime<180 && peak.charge<200 ){
            //     // cout << "Very small peak found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" charge:"<<peak.charge<<" risetime:"<<peak.risetime<< " loc:"<<peak.peakLoc<<endl;
            // }
            // if (peak.peakLoc<150 && peak.peakHeight<100 && peak.charge>3000){
            //     cout << "LowPeak with Large Charge found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" height:"<<peak.peakHeight<<" risetime:"<<peak.risetime<< " loc:"<<peak.peakLoc<<endl;
            // }
            // if ( peak.charge/peak.peakHeight>20){
            //     cout << "peak in large area found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" charge:"<<peak.charge<<" height:"<<peak.peakHeight<<" ratio:"<<peak.charge/peak.peakHeight<<" risetime:"<<peak.risetime<<" loc:"<<peak.peakLoc<<endl;
            // }
            if (peak.charge<500){
                cout << "Small charge peak found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" charge:"<<peak.charge<<" height:"<<peak.peakHeight<<" risetime:"<<peak.risetime<<" loc:"<<peak.peakLoc<<endl;
            }
            
            // if (peak.charge<300 && RunNo==51195 && peak.channel!=10 && peak.channel!=32){
            //     cout << "Very early peak found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch:"<<peak.channel<<" charge:"<<peak.charge<<" height:"<<peak.peakHeight<<" risetime:"<<peak.risetime<< " loc:"<<peak.peakLoc<<endl;
            // }
        }
        if (darknoise.size()<=0) continue;
        if (darknoise.size()>=2){
            multichcount++;
            // cout << "Multi-channel dn found in run "<<RunNo<<",triggerNo"<<TriggerNo<<". ch1:"<<darknoise[0].first<<" charge1:"<<darknoise[0].second<<", ch2:"<<darknoise[1].first<<" charge2:"<<darknoise[1].second<<endl;
            continue;  
        } 
        singlechcount++;
