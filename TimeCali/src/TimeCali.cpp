...
void analyze_darknoise(const char* filename, const char* outputpath, const char* histfile) {
    ...
    // 创建直方图
    TH2D* hRisetime = new TH2D("hRisetime", "Rise Time Distribution;Channel;Rise Time (ns)",
                               60, 0, 60, 1700, 315, 330);

    // 为每个通道创建一维直方图
    vector<TH1D*> histsPerChannel;
    for (int ch = 0; ch < 60; ++ch) {
        histsPerChannel.push_back(new TH1D(Form("hRise_Ch%d", ch), 
                                    Form("Rise Time Channel %d;Rise Time (ns);Counts", ch),
                                    1700, 315, 330));
    }
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        ...
        if (Getrmax(*peaks) > 0.3) {
            continue; // 跳过最大电荷与总电荷比值大于0.5的事例
        }
        // 处理每个峰值
        for (size_t j = 0; j < peaks->size(); j++) {
            ...
            if (channel >= 0 && channel < 60) {
                hRisetime->Fill(channel, risetime);
                histsPerChannel[channel]->Fill(risetime);
            }
        }
    ...
    for (int ch = 0; ch < 60; ++ch) {
        ...
        if (entries > 50) { // 确保有足够的数据点
            TF1* gausFit = new TF1(Form("gausFit_Ch%d", ch), "gaus", 50, 300);
            hist->Fit(gausFit, "LS RQ","",165,210); // R: 使用直方图范围, Q: 不输出拟合结果 LS:least squares
